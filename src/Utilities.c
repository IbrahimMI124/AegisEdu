...
int authenticate(int client_socket, int role)
{
    ...
    {
    case 1: // Admin
        if (strcmp(username, ADMIN) == 0 && strcmp(password, ADMIN_PWD) == 0)
        {
    ...
    if (found)
    {
        strcpy(buffer, "Authentication autherized.\n");
        send_message(client_socket, buffer);
        return id;
    }
    else
...
{
    char buffer[BUFFER_SIZE];
    strcpy(buffer, "Exiting Academia Portal.\nThank You.MSGKL\n");
    send_message(client_socket, buffer);
}
...
void send_message_and_prompt(int socket, const char *message)
{
    ...
    send(socket, "MSGRPLY", 7, 0); // Always send marker at end
    fflush(stdout);
}
...
{
    send(socket, message, strlen(message), 0);
    send(socket, "MSGTRM", 6, 0); // Always send marker at end
    fflush(stdout);
}
...
int recv_message(int socket, char *buffer)
{
    ...
    while (1)
    {
        ...
        char *end_ptr = strstr(static_buffer, "MSGTRM");
        char *expect_ptr = strstr(static_buffer, "MSGRPLY");
        char *exit_ptr = strstr(static_buffer, "MSGKL");
            ...
            (expect_ptr == NULL || exit_ptr < expect_ptr))
        {
            *exit_ptr = '\0';
            ...
            int remaining = buffer_filled - (exit_ptr - static_buffer + 5); 
            memmove(static_buffer, exit_ptr + 5, remaining);
                 ...
                 (expect_ptr == NULL || end_ptr < expect_ptr))
        {
            *end_ptr = '\0';
            ...
            int remaining = buffer_filled - (end_ptr - static_buffer + 6); 
            memmove(static_buffer, end_ptr + 6, remaining);
            static_buffer[remaining] = '\0';
            buffer_filled = remaining;
        ...
        else if (expect_ptr != NULL)
        {
            *expect_ptr = '\0';
            ...
            int remaining = buffer_filled - (expect_ptr - static_buffer + 7); 
            memmove(static_buffer, expect_ptr + 7, remaining);
            static_buffer[remaining] = '\0';
            buffer_filled = remaining;
